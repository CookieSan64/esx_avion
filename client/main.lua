...
function OpenAvionShop(shop)
	isInShopMenu = true

	local playerPed = PlayerPedId()
	local elements  = {
		{unselectable = true, icon = "fas fa-ship", title = TranslateCap('avion_shop')}
	}
				...
				ESX.Game.SpawnLocalVehicle(element.model, shop.Inside, shop.Inside.w, function (vehicle)
					...
					ESX.OpenContext("right", elements3, function(menu3,element3)
						if element3.value == "buy" then
							...
							ESX.TriggerServerCallback('esx_avion:buyAvion', function(bought)
								if bought then
									ESX.ShowNotification(TranslateCap('avion_shop_bought', element.name, ESX.Math.GroupDigits(element.price)))
									...
									ESX.ShowNotification(TranslateCap('avion_shop_nomoney'))
								end
							end, props)
...
function OpenAvionGarage(garage)
	ESX.TriggerServerCallback('esx_avion:getGarage', function (ownedAvions)
		if #ownedAvions == 0 then
			ESX.ShowNotification(TranslateCap('garage_noavions'))
		else
			-- get all available avions
			local elements = {
				{unselectable = true, icon = "fas fa-ship", title = TranslateCap('garage')}
			}
			for i=1, #ownedAvions, 1 do
				ownedAvions[i] = json.decode(ownedAvions[i])

				elements[#elements+1] = {
					icon = "fas fa-ship",
					title = getVehicleLabelFromHash(ownedAvions[i].model),
					vehicleProps = ownedAvions[i]
				}
			end
			...
			ESX.OpenContext("right", elements, function(menu,element)
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
					TriggerServerEvent('esx_avion:takeOutVehicle', vehicleProps.plate)
...
function OpenLicenceMenu(shop)
	local elements = {
		...
		{icon = "fas fa-ship", title = "Purchase Avion License"}
	}
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		ESX.OpenContext("right", elements2, function(menu2,element2)
			...
			ESX.TriggerServerCallback('esx_avion:buyAvionLicense', function (boughtLicense)
				if boughtLicense then
					ESX.ShowNotification(TranslateCap('license_bought', ESX.Math.GroupDigits(Config.LicensePrice)))
					ESX.CloseContext()

					OpenAvionShop(shop) -- parse current shop
				else
...
function StoreAvionInGarage(vehicle, teleportCoords)
	local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)

	ESX.TriggerServerCallback('esx_avion:storeVehicle', function (rowsChanged)
